...
export function createCard(pokemon: Pokemon): HTMLDivElement {
  ...
  const statsList = pokemon.stats.map((stat: PokemonStat) => 
    `<li class="col-md-6">${stat.stat.name}: <span class="fw-bold">${stat.base_stat}</span></li>`
  ).join('');

  const pokemonTypes = pokemon.types.map((typeObj: PokemonType) => typeObj.type.name);
    ...
    <div class="card-body">
        ...
        <h4 class="card-subtitle h5 text-center">
            ${pokemonTypes.map((type: string) => 
              `<span class="mx-2 rounded px-1 ${typeClasses[type] || ''}">${type}</span>`
            ).join('')}
        </h4>
    </div>
  `;
...
export function displayCardModel(cardModel: string): void {
